package cn.start.pp.framework.web.core.handler;
...
import static cn.start.pp.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        // 例如说：https://gitee.com/zhijiantianya/star-cloud/issues/ICSSRM、https://gitee.com/zhijiantianya/star-cloud/issues/ICT6FM
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            ...
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[报表模块 - 表结构未导入]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[工作流模块 - 表结构未导入]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[微信公众号模块 - 表结构未导入]");
        }
        // 4. 商城系统（已禁用）
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统模块 - 已禁用]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[商城系统模块 - 已禁用]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[ERP 模块 - 表结构未导入]");
        }
        // 6. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[CRM 模块 - 表结构未导入]");
        }
        // 7. 支付模块
        if (message.contains("pay_")) {
            log.error("[支付模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[支付模块 - 表结构未导入]");
        }
        // 8. AI 模块
        if (message.contains("ai_")) {
            log.error("[AI 模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[AI 模块 - 表结构未导入]");
        }
        // 9. IoT 模块
        if (message.contains("iot_")) {
            log.error("[IoT 模块 - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[IoT 模块 - 表结构未导入]");
        }
        return null;
    }

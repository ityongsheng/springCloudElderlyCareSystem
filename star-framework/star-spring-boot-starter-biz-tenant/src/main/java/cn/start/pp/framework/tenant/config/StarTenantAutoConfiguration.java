...
import cn.start.pp.framework.mybatis.core.util.MyBatisUtils;
import cn.start.pp.framework.redis.config.StarCacheProperties;
import cn.start.pp.framework.security.core.service.SecurityFrameworkService;
import cn.start.pp.framework.tenant.core.aop.TenantIgnore;
import cn.start.pp.framework.tenant.core.aop.TenantIgnoreAspect;
import cn.start.pp.framework.tenant.core.db.TenantDatabaseInterceptor;
import cn.start.pp.framework.tenant.core.job.TenantJobAspect;
import cn.start.pp.framework.tenant.core.mq.rabbitmq.TenantRabbitMQInitializer;
import cn.start.pp.framework.tenant.core.mq.redis.TenantRedisMessageInterceptor;
import cn.start.pp.framework.tenant.core.mq.rocketmq.TenantRocketMQInitializer;
import cn.start.pp.framework.tenant.core.redis.TenantRedisCacheManager;
import cn.start.pp.framework.tenant.core.security.TenantSecurityWebFilter;
import cn.start.pp.framework.tenant.core.service.TenantFrameworkService;
import cn.start.pp.framework.tenant.core.service.TenantFrameworkServiceImpl;
import cn.start.pp.framework.tenant.core.web.TenantContextWebFilter;
import cn.start.pp.framework.tenant.core.web.TenantVisitContextInterceptor;
...
public class StarTenantAutoConfiguration {
    ...
    public TenantFrameworkService tenantFrameworkService(TenantCommonApi tenantApi) {
        // 参见 https://gitee.com/zhijiantianya/star-cloud/issues/IC6YZF
        try {
     ...
     */
    @Configuration
    @ConditionalOnClass(name = "cn.start.pp.framework.mq.redis.core.RedisMQTemplate")
    ...
    public RedisCacheManager tenantRedisCacheManager(RedisTemplate<String, Object> redisTemplate,
                                                     ...
                                                     StarCacheProperties starCacheProperties,
                                                     TenantProperties tenantProperties) {
        // 创建 RedisCacheWriter 对象
        RedisConnectionFactory connectionFactory = Objects.requireNonNull(redisTemplate.getConnectionFactory());
        RedisCacheWriter cacheWriter = RedisCacheWriter.nonLockingRedisCacheWriter(connectionFactory,
                BatchStrategies.scan(starCacheProperties.getRedisScanBatchSize()));

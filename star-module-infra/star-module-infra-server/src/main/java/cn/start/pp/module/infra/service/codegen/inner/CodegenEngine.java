package cn.start.pp.module.infra.service.codegen.inner;
...
import cn.start.pp.framework.apilog.core.annotation.ApiAccessLog;
import cn.start.pp.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.start.pp.framework.common.util.date.LocalDateTimeUtils;
...
import cn.start.pp.framework.excel.core.annotations.DictFormat;
import cn.start.pp.framework.excel.core.convert.DictConvert;
import cn.start.pp.framework.excel.core.util.ExcelUtils;
...
import cn.start.pp.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 star-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 star-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.start.pp.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/star-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/star-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "star-module-${table.moduleName}/" + // 顶级模块
                "star-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "star-module-${table.moduleName}/" + // 顶级模块
                "star-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "star-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "star-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "star-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
